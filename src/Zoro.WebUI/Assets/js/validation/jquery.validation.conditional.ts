...
$.each(['requiredif', 'regularexpressionif', 'rangeif'], function (index, validationName) {
    ...
    // Hide form group
    $('[data-val-' + validationName + '-dependentproperty]').each(function (index, element: HTMLInputElement) {
        var prefix = "";
        var lastDot = element.name.lastIndexOf('.');
        if (lastDot != -1) {
        ...
        var targetvalue = <string>$(element).data('val-' + validationName + '-targetvalue') + "";
...
function getValue(control: JQuery): string {
